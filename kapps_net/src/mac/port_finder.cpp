...
namespace
{
bool isIpv4Local(const core::Ipv4Address &addr)
{
    return addr.isLoopback() || addr.isPrivateUse();
}

bool isIpv6Local(const core::Ipv6Address &addr, const core::Ipv6Address &networkPrefix)
{
    return addr.isLoopback() || addr.isLinkLocal() || addr.isUniqueLocalUnicast() ||
        ...
        (!networkPrefix.isNull() && addr.inSubnet(networkPrefix, 64));
}
...
void addressesForPid(pid_t pid, IPVersion ipVersion, Func_T func)
{
    ...
    for(const auto &fd : fds)
    {
        ...
        {
            KAPPS_CORE_WARNING() << "Failed to inspect descriptor" << fd.proc_fd << "of"
...
bool matchesPath(const std::vector<std::string> &paths, pid_t pid)
{
    ...
    return std::any_of(paths.begin(), paths.end(),
        [&appPath](const std::string &prefix) {
            ...
            // Also exclude empty prefixes (as they match everything)
            return !prefix.empty() && appPath.rfind(prefix, 0) == 0;
        });
}
...
pid_t pidForPort(std::uint16_t port, IPVersion ipVersion)
{
...
PortSet ports(const std::set<pid_t> &pids, IPVersion ipVersion, const OriginalNetworkScan &netScan)
{
    std::set<std::uint16_t> ports;
    for(const auto &pid : pids)
        addressesForPid(pid, ipVersion, [&](const auto &socketInfo) {
            ...
            {
                if(!isIpv4Local({socketInfo.remoteIp4()})) ports.insert(socketInfo.localPort());
            }
            else if(socketInfo.isIpv6())
            {
                // Find the on-link ipv6 network
                if(!isIpv6Local({socketInfo.remoteIp6()}, {netScan.ipAddress6()})) ports.insert(socketInfo.localPort());
            }
        });
...
PortSet ports(const std::vector<std::string> &paths, IPVersion ipVersion, const OriginalNetworkScan &netScan)
{

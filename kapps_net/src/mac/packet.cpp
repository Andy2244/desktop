...
core::nullable_t<Packet> Packet::createFromData(std::vector<unsigned char> data,
                                          unsigned skipBytes)
{
    // Must contain an IP header, we read these fields
    if(data.size() < skipBytes + sizeof(ip))
    {
        KAPPS_CORE_WARNING() << "IPv4 Packet of length" << data.size()
...
{
    const std::string sourceAddressStr = core::Ipv4Address { sourceAddress() }.toString();
    const std::string destAddressStr = core::Ipv4Address { destAddress() }.toString();

    if(packetType() != Other)
    {
        auto ptype = (packetType() == Udp ? "Udp" : packetType() == Tcp ? "Tcp" : "Other");
        return qs::format("Packet %: %:% -> %:%", ptype, sourceAddressStr, sourcePort(),
            destAddressStr, destPort());
    }
    else
        return qs::format("Packet (protocol: %1: % -> %", protocol(), sourceAddressStr, destAddressStr);
}
...
core::nullable_t<Packet6> Packet6::createFromData(std::vector<unsigned char> data,
                                            unsigned skipBytes)
{
    // Must contain an IPv6 header, we read these fields
    if(data.size() < skipBytes + sizeof(ip6_hdr))
    {
        KAPPS_CORE_WARNING() << "IPv6 Packet of length" << data.size()
    ...
    while(nextHeaderOffset)
    {
        ...
        switch(nextHeader)
        {
            ...
            case 44: // fragment
            {
                KAPPS_CORE_WARNING() << "Dropping IPv6 fragmented packet; fragments not supported";
                break;
            }
...
{
    const std::string sourceAddressStr = core::Ipv6Address { reinterpret_cast<const std::uint8_t*>(&sourceAddress()) }.toString();
    const std::string destAddressStr = core::Ipv6Address { reinterpret_cast<const std::uint8_t*>(&destAddress()) }.toString();

    if(packetType() != Other)
    {
        auto ptype = (packetType() == Udp ? "Udp" : packetType() == Tcp ? "Tcp" : "Other");
        return qs::format("Packet %: % port % -> % port %", ptype, sourceAddressStr, sourcePort(),
            destAddressStr, destPort());
    }
    else
        return qs::format("Packet (protocol: %): % -> %", protocol(), sourceAddressStr, destAddressStr);
}
